...
func TestParseMetric(t *testing.T) {
	tests := []struct {
		...
		{
			input:  "metric_metric{foo=\"bar\",bar=\"baz\"}",
			result: []Label{{"foo", "bar"}, {"bar", "baz"}},
			err:    nil,
		},
		{
	...
	for _, tc := range tests {
		result, err := ParseMetric(tc.input)
		if err != tc.err {
			t.Logf("error got: %v, wanted: %v", err, tc.err)
			t.Fail()
		}
