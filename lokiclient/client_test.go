...
func TestClient_Handle(t *testing.T) {
	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			c, err := New(cfg, nil, 0, log.NewNopLogger())
			require.NoError(t, err)
...
func TestClient_StopNow(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			cl, err := New(cfg, nil, 0, log.NewNopLogger())
			require.NoError(t, err)
...
func decompressFromBuffer(buffer *bytes.Buffer, maxSize int, compression CompressionType, sp opentracing.Span) ([]byte, error) {
	if len(buffer.Bytes()) > maxSize {
		return nil, fmt.Errorf("received message larger than max (%d vs %d)", len(buffer.Bytes()), maxSize)
	}
	switch compression {
	case NoCompression:
	...
	case RawSnappy:
		...
		if size > maxSize {
			return nil, fmt.Errorf("received message larger than max (%d vs %d)", size, maxSize)
		}
...
func decompressRequest(reader io.Reader, expectedSize, maxSize int, compression CompressionType, sp opentracing.Span) (body []byte, err error) {
	defer func() {
		if err != nil && len(body) > maxSize {
			err = fmt.Errorf("received message larger than max (%d vs %d)", len(body), maxSize)
		}
	}()
	if expectedSize > maxSize {
		return nil, fmt.Errorf("received message larger than max (%d vs %d)", expectedSize, maxSize)
	}
...
func Test_Tripperware(t *testing.T) {
	...
	require.NoError(t, err)
	var called bool
	c, err := NewWithTripperware(Config{
