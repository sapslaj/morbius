...
import (
	...
	"github.com/sapslaj/morbius/lokiclient/fake"
...
func TestNewMulti(t *testing.T) {
	_, err := NewMulti(nil, log.NewNopLogger(), 0, []Config{}...)
	if err == nil {
	...
	clients, err := NewMulti(nil, log.NewNopLogger(), 0, cc1, cc2)
	if err != nil {
...
func TestNewMulti_BlockDuplicates(t *testing.T) {
	...
	_, err = NewMulti(nil, log.NewNopLogger(), 0, cc1, cc1Copy)
	require.Error(t, err, "expected NewMulti to reject duplicate client configs")

	cc1Copy.Name = "copy"
	clients, err := NewMulti(nil, log.NewNopLogger(), 0, cc1, cc1Copy)
...
func TestMultiClient_Handle_Race(t *testing.T) {
	...
	c1, err := New(Config{URL: u, BackoffConfig: backoff.Config{MaxRetries: 1}, Timeout: time.Microsecond}, nil, 0, log.NewNopLogger())
	require.NoError(t, err)
	c2, err := New(Config{URL: u, BackoffConfig: backoff.Config{MaxRetries: 1}, Timeout: time.Microsecond}, nil, 0, log.NewNopLogger())
	require.NoError(t, err)
