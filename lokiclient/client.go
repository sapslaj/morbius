...
func New(cfg Config, streamLagLabels []string, maxStreams int, logger log.Logger) (Client, error) {
	...
	return newClient(cfg, streamLagLabels, maxStreams, logger)
}

func newClient(cfg Config, streamLagLabels []string, maxStreams int, logger log.Logger) (*client, error) {
...
func NewWithTripperware(cfg Config, streamLagLabels []string, maxStreams int, logger log.Logger, tp Tripperware) (Client, error) {
	c, err := newClient(cfg, streamLagLabels, maxStreams, logger)
	if err != nil {
		return nil, err
	}
...
func (c *client) send(ctx context.Context, tenantID string, buf []byte) (int, error) {
	...
	defer func() {
		if err := resp.Body.Close(); err != nil {
			c.logger.Log("message", "closing response body", "error", err)
		}
	}()
